...
* You do not knit shiny files, you run them (the knit button in R Studio is replaced with a run button once you add the runtime shiny argument). 
* Running shiny creates an object called **input** which stores the values from your input widgets. Whatever you assign for the input widget ID will be the address for the value, which you can then access at **input$your_id**. 


## Templates

R Shiny demo templates and a lab template are available here: 
...
**No Knitting to HTML this week:**
 
When you have completed your assignment, click the "Run App" button to ensure your dashboard functions. 

You **cannot** knit R Shiny apps to an HTML file the same you do for previous assignments. Dynamic graphics require javascript + data. As a result Shiny apps require separate library of code and the input data in order to makes the graphics interactive. These files are generated locally when you run the Shiny app on your computer, but running the app does not generate a self-contained HTML file that can be easily shared.   

**This week, submit your raw RMD file instead of HTML.**
