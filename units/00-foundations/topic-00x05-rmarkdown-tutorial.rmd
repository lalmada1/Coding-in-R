...
A full description of TeX math symbols is beyond the scope of this tutorial. Here's a [good cheatsheet](http://tug.ctan.org/info/undergradmath/undergradmath.pdf), and you can try stuff out on CodeCogs. You can also play with formulas in the `Markdown Here` options page.
...
The easiest way to embed videos is to click the "share" option on YouTube or a similar video platform and select the "embed" option, which will give you an iframe. 

Or else you can create traditional HTML code, which involves inserting an image with a hyperlink. The difference between the two is that iframes will allow you to watch the video from within the document, whereas the HTML option will take you to the video on a different page. 
...
_Note, if your iframe is not rendering as expected you might try adding the argument **data-external="1"**. Typically you can paste the copied iframe text directly from youtube or other hosting services and it works fine, but there have been some changes to how R Markdown renders iframes. [Adding the extra argument is a workaround](https://community.rstudio.com/t/insert-raw-html-iframe-into-rmarkdown/94334/2)._ 
